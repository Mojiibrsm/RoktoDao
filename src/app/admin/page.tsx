  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        // Note: This requires a `createdAt` field in your donor documents.
        // As it's not present, we will simulate by just counting all donors as "new" for now.
        // A proper implementation would look like this:
        const thirtyDaysAgo = new Date();
        thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
        // const signupsQuery = query(donorsCollection, where('createdAt', '>=', Timestamp.fromDate(thirtyDaysAgo)));
        // const signupsSnapshot = await getDocs(signupsQuery);
        // const newSignups = signupsSnapshot.size;
        // Since we dont have createdAt, using total donors as a placeholder for new signups
        const newSignups = totalDonors;
        ...
        setStats({
          ...
          activeRequests,
          newSignups,
          requestsFulfilled: 0, // This needs a dedicated logic/field to be calculated
        });
      } catch (error) {
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Total pending requests</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">In the last 30 days (simulated)</p>
          </CardContent>
        </Card>
      </div>
